...
import { HiOutlineBuildingLibrary, HiOutlineShare, HiOutlineChatBubbleLeftRight } from "react-icons/hi2";
import PageHeader from "../../components/PageHeader";
import FilterTabs from "../../components/FilterTabs";
import SearchInput from "../../components/SearchInput";
import Fab from "../../components/Fab";
import { govtPaymentAPI } from "../../api/erp";
...
export default function GovtList() {
    ...
    useEffect(() => {
        govtPaymentAPI.recent().then(setRows).finally(() => setLoading(false));
    }, []);
    ...
    const filtered = useMemo(() => rows.filter(match)
        .filter((r) => `${r.client} ${r.name}`.toLowerCase().includes(search.trim().toLowerCase())),
        [rows, filter, search]);
    ...
    return (
        <div className="page-container pb-28">
            <PageHeader title="Govt Payments" subtitle="Reimbursable govt fee vouchers" back="/" />
            <FilterTabs<Filter>
            ...
            {!loading && filtered.length === 0 && (
                <div className="text-center py-20">
                    <HiOutlineBuildingLibrary className="w-12 h-12 mx-auto mb-3 opacity-40" style={{ color: "var(--color-text-muted)" }} />
                    <p style={{ color: "var(--color-text-muted)" }}>No vouchers here</p>
                </div>
            )}
                ...
                {filtered.map((r) => (
                    <div key={r.name} className="glass-card p-4 flex items-center gap-3"
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="font-semibold truncate" style={{ color: "var(--color-text)" }}>{r.client}</p>
                            <p className="text-xs mt-0.5" style={{ color: "var(--color-text-muted)" }}>
                                {r.name} · {prettyDate(r.payment_date)} · <span style={{ color: r.docstatus === 2 ? "#f87171" : undefined }}>{statusLabel(r.docstatus)}</span>
                            </p>
                        </div>
                        <p className="font-bold shrink-0" style={{ color: "var(--color-text)" }}>{inr(r.total_amount)}</p>
                        <button onClick={() => sharePdf("Govt Payment Voucher", r.name, `Govt Payment ${r.name}\n${r.client}\nAmount: ${inr(r.total_amount)}`)}
                            className="p-2 rounded-lg shrink-0" style={{ color: "var(--color-text-secondary)" }} aria-label="Share PDF">
                            <HiOutlineShare className="w-5 h-5" />
                        </button>
                        <button onClick={() => sendViaEvo("Govt Payment Voucher", r.name, r.client, `Govt Payment ${r.name} · Amount ${inr(r.total_amount)}`)}
            ...
            <Fab to="/govt/new" label="New Voucher" />
        </div>
    );
}
